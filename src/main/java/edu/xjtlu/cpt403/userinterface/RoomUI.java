...
public class RoomUI {
    ...
    public static void newReservation() {
        ...
        Room room;
        try {
            Room.checkReservation(Integer.parseInt(roomNumber), DataBaseManager.getRoomDAO());
            //  获取到room的信息， 便于之后使用
        ...
        int userId;
        // 用户没有登录（当前用户=null）， 或者用户是管理员， 则需要提示输入需要预定房间的用户名字
    ...
    public static void cancelReservation() {
        ...
        // 4. 检测当前用户是不是管理员，
        User currentUser = UserInterfaceUtils.getCurrentUser();
        // 用户未登录（当前用户 = null），不可以取消预定
        if (currentUser == null) {
            System.out.println("current user is empty, so can't support cancel reservation!");
            return;
        }
